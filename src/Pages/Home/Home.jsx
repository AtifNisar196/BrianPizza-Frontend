...
const Home = () => {
  ...
  useEffect(() => {
    const fetchFeaturedProducts = async () => {
      try {
        setIsOpen(false);
        const response = await axios.get(`${apiUrl}/featured-products`);
  ...
  return (
    <div className="">
      ...
      <section className="middle-sections">
        <Banner
          title={"We believe that good food improves mood"}
            ...
            >
              {featuredProducts.map((featuredproduct, index) => (
                <SwiperSlide key={index}>
                  ...
                  <Link
                                  to="/menu-product"
                                  state={{ from: featuredproduct.id }}
                                  style={{textDecoration:"none"}}
                                >
                    <div className="top-selling-img">
                      <img src={featuredproduct.image} alt="Slider Image" />
                    </div>
                    <div className="top-selling-content">
                      <h4>{featuredproduct.name}</h4>
                      <p>{featuredproduct.description.substring(0, 40)}</p>
                      <p>Total Cost: ${featuredproduct.price}</p>
